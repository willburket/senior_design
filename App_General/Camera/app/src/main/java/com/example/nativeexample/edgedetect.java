...
public class edgedetect extends AppCompatActivity {
    ...
    private DisparityTransformFilter mDepthtoDisparity;
    private DisparityTransformFilter mDisparitytoDepth;
    ...
    private void init(){
        ...
        mDisparitytoDepth = new DisparityTransformFilter(false);
        mDepthtoDisparity = new DisparityTransformFilter(true);

        //config filters
        mThresholdFilter.setValue(Option.MIN_DISTANCE, 0.1f);           //distance threshold min
        mThresholdFilter.setValue(Option.MAX_DISTANCE, 0.8f);           //max

        mDecimationFilter.setValue(Option.FILTER_MAGNITUDE, 8);         //decimation config

        try(DeviceList dl = mRsContext.queryDevices()){
            if(dl.getDeviceCount() > 0) {                       // start if device is connected
        ...
        public void run() {
            try {
                try(FrameReleaser fr = new FrameReleaser()){
                    ...
                    FrameSet orgSet = frames.applyFilter(mColorizerOrg).releaseWith(fr);            // colorized depth
                          //  applyFilter(mDecimationFilter).releaseWith(fr).                       // processed (no edge preserve)
                    ...
                    FrameSet processedSet = frames.applyFilter(mDecimationFilter).releaseWith(fr).      //edge preserved set
                            applyFilter(mThresholdFilter).releaseWith(fr).
                            applyFilter(mDepthtoDisparity).releaseWith(fr).                         //depth to disparity
                            applyFilter(mSpatialFilter).releaseWith(fr).
                            applyFilter(mTemporalFilter).releaseWith(fr).
                            applyFilter(mDisparitytoDepth).releaseWith(fr).
                            applyFilter(mHoleFillingFilter).releaseWith(fr).    //disparity to depth
                            applyFilter(mColorizerProcessed).releaseWith(fr).
                            applyFilter(mAlign).releaseWith(fr);
